####Generate data for figure 3 main text and figure s8 supp ############
rm(list=ls())
library(deSolve)
library(tidyverse)
library(rootSolve)
...
##########observed data#####################
load("Datasummary2/obs_sero")
df_obs<-obs_sero
#############load model output and plot chain####
load("Data/Nationmod5")
df_chain=Nationmod5$chain[10000:50000,]
...
numSamples = 1000
###################RC per year figure s2##############
outDf_RCall2<- matrix(NA,nrow=numSamples, ncol = 28)
for (mm in 1:numSamples ) {
  ...
  df_RCall2=df_RCall %>%dplyr::select(!c(Agegroup,Age_year)) %>%
    group_by(Year) %>%
  ...
  outDf_RCall2[mm,] <-  predc_sero.sample2
}

## for each row in the matrix get quantiles
quantileMatrix <- matrix(NA,nrow=ncol(outDf_RCall2), ncol = 3)
for(jj in 1:ncol(outDf_RCall2)){
  quantiles <- outDf_RCall2[,jj] %>% quantile(probs=c(.5,.025,.975))
...
predRC_allgps=predRC_allgps %>%
  mutate(Immunity_status=c(rep("MCV1",7),
                           rep("MCV2",7),
                           rep("SIAs",7),
                           rep("Natural Infection",7))) %>% 
  mutate(Year=rep(seq(2009,2021,2),4))

predRC_allgps$Year=factor(predRC_allgps$Year,levels=unique(predRC_allgps$Year))
predRC_allgps$Immunity_status=factor(predRC_allgps$Immunity_status,
                                     levels=c("MCV1","MCV2","SIAs","Natural Infection"))

predRC_yearlybase=predRC_allgps
#Save the data
save(predRC_yearlybase,file = "DATA_STEF_REV/predRC_yearlybase")


###################RC overall main figure2##############
...
predRC_allbase=predRC_allgps
#Save the data
save(predRC_allbase,file = "DATA_STEF_REV/predRC_allbase")

########The following is the RC in all the 5 scenarios+baseline looking at only years after MCV2 to form main figure 3#####
###################RC contribution of the mcv2a scenario,MCV2=MCV1########
df_chain=Nationmod5$chain[10000:50000,]
df_chain= df_chain%>% as.data.frame()
names(df_chain) =c("Beta1","Beta2","Beta3","omega")
df_chain_new=df_chain
mcmcMatrix=as.matrix(df_chain_new)
numSamples = 1000
outmcv2a_RC <- matrix(NA,nrow=numSamples, ncol = 5)
for (kk in 1:numSamples ) {
  ...
  params.list14=list(
    noagegps=noagegps,
    alpha_v1=c(rep(0,10),cov14,rep(0,25)),
    alpha_v2=c(rep(0,17),cov14,rep(0,18)),  
    alpha_v3=c(rep(0,36)),
    ageout=ageing,
    casedf=c(rep(0,6),rep(case14,30)),
    omega=omegahi,
  ...
  params.list15=list(
    noagegps=noagegps,
    alpha_v1=c(rep(0,10),cov15,rep(0,25)),
    alpha_v2=c(rep(0,17),cov15,rep(0,18)),   
    alpha_v3=c(rep(0,36)),
    ageout=ageing,
    casedf=c(rep(0,6),rep(case15,30)),
    omega=omegahi,
  ...
  params.list16=list(
    noagegps=noagegps,
    alpha_v1=c(rep(0,10),cov16,rep(0,25)),
    alpha_v2=c(rep(0,17),cov16,rep(0,18)),    
    alpha_v3=c(rep(0,36)),
    ageout=ageing,
    casedf=c(rep(0,6),rep(case16,30)),
    omega=omegahi,
  ...
  params.list17=list(
    noagegps=noagegps,
    alpha_v1=c(rep(0,10),cov17,rep(0,25)),
    alpha_v2=c(rep(0,17),cov17,rep(0,18)),   
    alpha_v3=c(rep(0,36)),
    ageout=ageing,
    casedf=c(rep(0,6),rep(case17,30)),
    omega=omegahi,
  ...
  params.list18=list(
    noagegps=noagegps,
    alpha_v1=c(rep(0,10),cov18,rep(0,25)),
    alpha_v2=c(rep(0,17),cov18,rep(0,18)),    
    alpha_v3=c(rep(0,36)),
    ageout=ageing,
    casedf=c(rep(0,6),rep(case18,30)),
    omega=omegahi,
  ...
  params.list19=list(
    noagegps=noagegps,
    alpha_v1=c(rep(0,10),cov19,rep(0,25)),
    alpha_v2=c(rep(0,17),cov19,rep(0,18)),    
    alpha_v3=c(rep(0,36)),
    ageout=ageing,
    casedf=c(rep(0,6),rep(case19,30)),
    omega=omegahi,
  ...
  params.list20=list(
    noagegps=noagegps,
    alpha_v1=c(rep(0,10),cov20,rep(0,25)),
    alpha_v2=c(rep(0,17),cov20,rep(0,18)),    
    alpha_v3=c(rep(0,36)),
    ageout=ageing,
    casedf=c(rep(0,6),rep(case20,30)),
    omega=omegahi,
  ...
  params.list21=list(
    noagegps=noagegps,
    alpha_v1=c(rep(0,10),cov21,rep(0,25)),
    alpha_v2=c(rep(0,17),cov21,rep(0,18)),  
    alpha_v3=c(rep(0,36)),
    ageout=ageing,
    casedf=c(rep(0,6),rep(case21,30)),
    omega=omegahi,
  ...
  predc_sero.MCV1=df_RCall2[,"MCV1"]/rowSums(df_RCall2[,c("MCV1","MCV2","SIA","NI","S")])
  predc_sero.MCV2=df_RCall2[,"MCV2"]/rowSums(df_RCall2[,c("MCV1","MCV2","SIA","NI","S")])
  predc_sero.SIA=df_RCall2[,"SIA"]/rowSums(df_RCall2[,c("MCV1","MCV2","SIA","NI","S")])
  predc_sero.NI=df_RCall2[,"NI"]/rowSums(df_RCall2[,c("MCV1","MCV2","SIA","NI","S")])
  predc_sero.S=df_RCall2[,"S"]/rowSums(df_RCall2[,c("MCV1","MCV2","SIA","NI","S")])
  #predc_sero.MI=df_RCall2[,"MI"]/rowSums(df_RCall2[,c("MCV1","MCV2","SIA","NI","S")])
  
  predc_sero.sample=cbind(predc_sero.MCV1,predc_sero.MCV2,predc_sero.SIA,predc_sero.NI,predc_sero.S)
  
  predc_sero.sample2=as.vector(predc_sero.sample) %>% unlist()
  
  outmcv2a_RC[kk,] <-  predc_sero.sample2
}
## for each row in the matrix get quantiles
quantileMatrix <- matrix(NA,nrow=ncol(outmcv2a_RC), ncol = 3)
for(jj in 1:ncol(outmcv2a_RC)){
  quantiles <- outmcv2a_RC[,jj] %>% quantile(probs=c(.5,.025,.975))
  quantileMatrix[jj,] <- quantiles
}
...
colnames(predRC_allgps)=c("midx","lox","hix")
predRC_allgps=predRC_allgps %>%
  mutate(Immunity_status=c(rep("MCV1",1),
                           rep("MCV2",1),
                           rep("SIAs",1),
                           rep("Natural Infection",1),
                           rep("Susceptible",1)))


predRC_allgps$Immunity_status=factor(predRC_allgps$Immunity_status,levels=c("MCV1","Natural Infection",
                                                                            "SIAs","Susceptible","MCV2"))


#Save the data
RC_mcv2aonly=predRC_allgps
save(RC_mcv2aonly,file = "Finaldatasummary2/RC_mcv2aonly")

########################RC MCV2b scenario(halfway between)##########
df_chain=Nationmod5$chain[10000:50000,]
df_chain= df_chain%>% as.data.frame()
names(df_chain) =c("Beta1","Beta2","Beta3","omega")
df_chain_new=df_chain
mcmcMatrix=as.matrix(df_chain_new)
numSamples = 1000
outmcv2b_RC <- matrix(NA,nrow=numSamples, ncol = 5)
for (kk in 1:numSamples ) {
  randomNumber <- floor(runif(1, min = 1, max = nrow(mcmcMatrix)))
  ...
  case14=Case_data[19,3] %>% pull("Cases")
  case14=case14
  cov_14=MCV2_data[19,3] %>% pull("MCV2")
  cov2_14=cov_14+(cov14-cov_14)*0.5
  start14<-as.data.frame(t(model13[13,-1])) %>% pull("13")
  params.list14=list(
    noagegps=noagegps,
    alpha_v1=c(rep(0,10),cov14,rep(0,25)),
  ...
  cov15=cov15
  case15=Case_data[20,3] %>% pull("Cases")
  cov_15=MCV2_data[20,3] %>% pull("MCV2")
  cov2_15=cov_15+(cov15-cov_15)*0.5
  start15<-as.data.frame(t(model14[13,-1])) %>% pull("13")
  params.list15=list(
    noagegps=noagegps,
    alpha_v1=c(rep(0,10),cov15,rep(0,25)),
  ...
  cov16=cov16
  case16=Case_data[21,3] %>% pull("Cases")
  cov_16=MCV2_data[21,3] %>% pull("MCV2")
  cov2_16=cov_16+(cov16-cov_16)*0.5
  ...
  cov17=cov17
  case17=Case_data[22,3] %>% pull("Cases")
  cov_17=MCV2_data[22,3] %>% pull("MCV2")
  cov2_17=cov_17+(cov17-cov_17)*0.5
  start17<-as.data.frame(t(model16[13,-1])) %>% pull("13")
  params.list17=list(
    noagegps=noagegps,
    alpha_v1=c(rep(0,10),cov17,rep(0,25)),
  ...
  cov18=cov18
  case18=Case_data[23,3] %>% pull("Cases")
  cov_18=MCV2_data[23,3] %>% pull("MCV2")
  cov2_18=cov_18+(cov18-cov_18)*0.5
  ...
  cov19=cov19
  case19=Case_data[24,3] %>% pull("Cases")
  cov_19=MCV2_data[24,3] %>% pull("MCV2")
  cov2_19=cov_19+(cov19-cov_19)*0.5
  ...
  cov20=cov20
  case20=Case_data[25,3] %>% pull("Cases")
  cov_20=MCV2_data[25,3] %>% pull("MCV2")
  cov2_20=cov_20+(cov20-cov_20)*0.5
  ...
  cov21=cov21
  case21=Case_data[26,3] %>% pull("Cases")
  cov_21=MCV2_data[26,3] %>% pull("MCV2")
  cov2_21=cov_21+(cov21-cov_21)*0.5
  ...
  outmcv2b_RC[kk,] <-  predc_sero.sample2
}
## for each row in the matrix get quantiles
quantileMatrix <- matrix(NA,nrow=ncol(outmcv2b_RC), ncol = 3)
for(jj in 1:ncol(outmcv2b_RC)){
  quantiles <- outmcv2b_RC[,jj] %>% quantile(probs=c(.5,.025,.975))
  quantileMatrix[jj,] <- quantiles
}
...
#Save the data
RC_mcv2bonly=predRC_allgps
save(RC_mcv2bonly,file = "Finaldatasummary2/RC_mcv2bonly")

#########RC MCV1 scenario,10% MCV1 increase(11mnths),MCV2=MCV1##########
df_chain=Nationmod5$chain[10000:50000,]
df_chain= df_chain%>% as.data.frame()
names(df_chain) =c("Beta1","Beta2","Beta3","omega")
df_chain_new=df_chain
mcmcMatrix=as.matrix(df_chain_new)
numSamples = 1000
outmcv1a_RC <- matrix(NA,nrow=numSamples, ncol = 5)
for (kk in 1:numSamples ) {
  randomNumber <- floor(runif(1, min = 1, max = nrow(mcmcMatrix)))
  ...
  outmcv1a_RC[kk,] <-  predc_sero.sample2
}
## for each row in the matrix get quantiles
quantileMatrix <- matrix(NA,nrow=ncol(outmcv1a_RC), ncol = 3)
for(jj in 1:ncol(outmcv1a_RC)){
  quantiles <- outmcv1a_RC[,jj] %>% quantile(probs=c(.5,.025,.975))
  quantileMatrix[jj,] <- quantiles
}
...
#Save the data
RC_mcv1mcv2a=predRC_allgps
save(RC_mcv1mcv2a,file = "Finaldatasummary2/RC_mcv1mcv2a")

#########RC MCV1 scenario,10% MCV1 increase(9mnths),MCV2=MCV1##########
df_chain=Nationmod5$chain[10000:50000,]
df_chain= df_chain%>% as.data.frame()
names(df_chain) =c("Beta1","Beta2","Beta3","omega")
df_chain_new=df_chain
mcmcMatrix=as.matrix(df_chain_new)
numSamples = 1000
outmcv1b_RC <- matrix(NA,nrow=numSamples, ncol = 5)
for (kk in 1:numSamples ) {
  randomNumber <- floor(runif(1, min = 1, max = nrow(mcmcMatrix)))
  ...
  outmcv1b_RC[kk,] <-  predc_sero.sample2
}
## for each row in the matrix get quantiles
quantileMatrix <- matrix(NA,nrow=ncol(outmcv1b_RC), ncol = 3)
for(jj in 1:ncol(outmcv1b_RC)){
  quantiles <- outmcv1b_RC[,jj] %>% quantile(probs=c(.5,.025,.975))
  quantileMatrix[jj,] <- quantiles
}
...
#Save the data
RC_mcv1mcv2b=predRC_allgps
save(RC_mcv1mcv2b,file = "Finaldatasummary2/RC_mcv1mcv2b")

#########RC MCV1 scenario,95% MCV1 (9mnths) & 95% MCV2(18mnths)##########
df_chain=Nationmod5$chain[10000:50000,]
df_chain= df_chain%>% as.data.frame()
names(df_chain) =c("Beta1","Beta2","Beta3","omega")
df_chain_new=df_chain
mcmcMatrix=as.matrix(df_chain_new)
numSamples = 1000
outmcv1c_RC <- matrix(NA,nrow=numSamples, ncol = 5)
for (kk in 1:numSamples ) {
  randomNumber <- floor(runif(1, min = 1, max = nrow(mcmcMatrix)))
  ...
  outmcv1c_RC[kk,] <-  predc_sero.sample2
}
## for each row in the matrix get quantiles
quantileMatrix <- matrix(NA,nrow=ncol(outmcv1c_RC), ncol = 3)
for(jj in 1:ncol(outmcv1c_RC)){
  quantiles <- outmcv1c_RC[,jj] %>% quantile(probs=c(.5,.025,.975))
  quantileMatrix[jj,] <- quantiles
}
...
#Save the data
RC_mcv1mcv2c=predRC_allgps
save(RC_mcv1mcv2c,file = "Finaldatasummary2/RC_mcv1mcv2c")
################baseline for entire pop only in years with MCV2###########
df_chain=Nationmod5$chain[10000:50000,]
df_chain= df_chain%>% as.data.frame()
names(df_chain) =c("Beta1","Beta2","Beta3","omega")
df_chain_new=df_chain
mcmcMatrix=as.matrix(df_chain_new)
numSamples = 1000
outbasenew_RC <- matrix(NA,nrow=numSamples, ncol = 6)
for (kk in 1:numSamples ) {
  randomNumber <- floor(runif(1, min = 1, max = nrow(mcmcMatrix)))
  ...
  predc_sero.MCV1=df_RCall2[,"MCV1"]/rowSums(df_RCall2)
  predc_sero.MCV2=df_RCall2[,"MCV2"]/rowSums(df_RCall2)
  predc_sero.SIA=df_RCall2[,"SIA"]/rowSums(df_RCall2)
  predc_sero.NI=df_RCall2[,"NI"]/rowSums(df_RCall2)
  predc_sero.S=df_RCall2[,"S"]/rowSums(df_RCall2)
  predc_sero.MI=df_RCall2[,"MI"]/rowSums(df_RCall2)
  
  predc_sero.sample=cbind(predc_sero.MCV1,predc_sero.MCV2,predc_sero.SIA,predc_sero.NI,predc_sero.S,predc_sero.MI)
  
  predc_sero.sample2=as.vector(predc_sero.sample) %>% unlist()
  
  outbasenew_RC[kk,] <-  predc_sero.sample2
}
## for each row in the matrix get quantiles
quantileMatrix <- matrix(NA,nrow=ncol(outbasenew_RC), ncol = 3)
for(jj in 1:ncol(outbasenew_RC)){
  quantiles <- outbasenew_RC[,jj] %>% quantile(probs=c(.5,.025,.975))
  quantileMatrix[jj,] <- quantiles
}
...
predRC_allgps=predRC_allgps %>%
  mutate(Immunity_status=c(rep("MCV1",1),
                           ...
                           rep("Natural Infection",1),
                           rep("Susceptible",1),
                           rep("Maternal Immunity",1)))

predRC_allgps$Immunity_status=factor(predRC_allgps$Immunity_status,levels=c("MCV1","Natural Infection",
                                                                            "SIAs","Susceptible","Maternal Immunity",
                                                                            "MCV2"))

#Save the data
RC_newbase=predRC_allgps
save(RC_newbase,file = "Finaldatasummary2/RC_newbase")


################baseline for seroconverted only in years with MCV2###########
df_chain=Nationmod5$chain[10000:50000,]
df_chain= df_chain%>% as.data.frame()
names(df_chain) =c("Beta1","Beta2","Beta3","omega")
df_chain_new=df_chain
mcmcMatrix=as.matrix(df_chain_new)
numSamples = 1000
outbasenew2_RC <- matrix(NA,nrow=numSamples, ncol = 4)
for (kk in 1:numSamples ) {
  randomNumber <- floor(runif(1, min = 1, max = nrow(mcmcMatrix)))
  ...
  outbasenew2_RC[kk,] <-  predc_sero.sample2
}
## for each row in the matrix get quantiles
quantileMatrix <- matrix(NA,nrow=ncol(outbasenew2_RC), ncol = 3)
for(jj in 1:ncol(outbasenew2_RC)){
  quantiles <- outbasenew2_RC[,jj] %>% quantile(probs=c(.5,.025,.975))
  quantileMatrix[jj,] <- quantiles
}
...
predRC_allgps$Immunity_status=factor(predRC_allgps$Immunity_status,levels=c("MCV1","Natural Infection","SIAs","MCV2"))

#Save the data
RC_newbase2=predRC_allgps
save(RC_newbase2,file = "Finaldatasummary2/RC_newbase2")

################baseline for entire pop without immunity comp only in years with MCV2###########
df_chain=Nationmod5$chain[10000:50000,]
df_chain= df_chain%>% as.data.frame()
names(df_chain) =c("Beta1","Beta2","Beta3","omega")
df_chain_new=df_chain
mcmcMatrix=as.matrix(df_chain_new)
numSamples = 1000
outbasenew_RC3 <- matrix(NA,nrow=numSamples, ncol = 5)
for (kk in 1:numSamples ) {
  randomNumber <- floor(runif(1, min = 1, max = nrow(mcmcMatrix)))
  ...
  outbasenew_RC3[kk,] <-  predc_sero.sample2
}
## for each row in the matrix get quantiles
quantileMatrix <- matrix(NA,nrow=ncol(outbasenew_RC3), ncol = 3)
for(jj in 1:ncol(outbasenew_RC3)){
  quantiles <- outbasenew_RC3[,jj] %>% quantile(probs=c(.5,.025,.975))
  quantileMatrix[jj,] <- quantiles
}
...
#Save the data
RC_newbase3=predRC_allgps
save(RC_newbase3,file = "Finaldatasummary2/RC_newbase3")
